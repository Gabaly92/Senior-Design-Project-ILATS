/*------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------*/
/*-----------------Battery Variables---------------------*/
unsigned char green = 0x08;
unsigned char lightblue = 0x0C;
unsigned char blue = 0x04;
unsigned char off = 0x00;
unsigned char Switch = 8;
int battVolts;
// functions
/*------------------Battery Functions---------------------------*/
void Battery_RGB(void)
{
    long Ontime;
    // Get Battery Value
    ...
    // Show Range color on LED
    if( (battVolts >= 400) && (battVolts < 500) )
    {
      PORTD = green;
      delay(2000);
      PORTD = off;
    }
    else if ( (battVolts >= 200) && (battVolts < 400) )
    {
      PORTD = lightblue;
      delay(2000);
      PORTD = off;
    }
    else if ( (battVolts >= 0) && (battVolts < 200) )
    {
      PORTD = blue;
      delay(2000);
      PORTD = off;  
...
void loop()
{
  ...
  //dX = abs((double) x / 64.0);          // calculate the 'g' values.
  //dY = abs((double) y / 64.0);
  //dZ = abs((double) z / 64.0);
  ...
  // battery
  if( digitalRead(Switch) == HIGH && dX < 0.13 && dY < 0.13)
  {
    Battery_RGB();
  }
